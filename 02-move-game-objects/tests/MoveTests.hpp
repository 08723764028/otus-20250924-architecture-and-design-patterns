...
TEST(Move, FromOnePointToAnotherUsingVelocity) {
  ...
  move.Execute();
  auto is_expected_result = movable_ship_adapter.GetLocation() == expected_position;
  EXPECT_EQ(is_expected_result, true);
}

TEST(Move, NoValidObjectPassedForExecution) {
  SCOPED_TRACE("Throw an exception if no valid object is passed to move command");
  Move move { nullptr };
