...
#include "RotableSpaceShip.hpp"
#include "MockedRotable.hpp"
#include "gtest/gtest.h"

TEST(Rotate, RotateWithSomeAngularVelocity) {
  SCOPED_TRACE("Check direction change from 1 to 3 with angular velocity 2 and 8 directions");
  SpaceShipProps ship { 
    .directions_number = 8,
    .direction = 1,
    .angular_velocity = 2 };
...
TEST(Rotate, RotateWithDirectionOverflow) {
  SCOPED_TRACE("Check direction change from 1 to 3 with angular velocity 10 and 8 directions");
  SpaceShipProps ship { 
    .directions_number = 8,
    .direction = 1,
    .angular_velocity = 10 };
...
TEST(Rotate, RotateWithDirectionZeroValue) {
  SCOPED_TRACE("Check for floating point exception if directions_number is zero");
  SpaceShipProps ship { 
    .directions_number = 8,
    .direction = 0,
    .angular_velocity = 10 };
  ...
  try {
    ...
    auto is_expected_result = err_msg == std::string("GetDirectionsNumberException: zero value is forbidden");
...
TEST(Rotate, GetAngularVelocityThrowsException) {
  ...
  try {
    ...
    std::string err_msg { e.what() };
    auto is_expected_result = err_msg == std::string("GetAngularVelocityException");
    EXPECT_TRUE(is_expected_result);
  }
}
...
TEST(Rotate, GetDirectionsNumberThrowsException) {
  ...
  try {
    ...
    std::string err_msg { e.what() };
    auto is_expected_result = err_msg == std::string("GetDirectionsNumberException");
    EXPECT_TRUE(is_expected_result);
  }
}
